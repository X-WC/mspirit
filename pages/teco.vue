...
<template>
  <div>
    <div class="px-4 md:pl-28 pt-6 md:pt-10 pb-8 md:pb-14 bg-no-repeat bg-cover bg-center"
      ...
      <h1 class="font-semibold text-3xl md:text-5xl m-0">TECHNOLOGY</h1>
      <h2 class="font-semibold text-2xl md:text-4xl m-0">核心技术</h2>
    ...
    <template v-if="selected === 0">
      <div style="min-height: 463px;"
           ...
           class="text-white flex flex-col md:flex-row justify-around gap-6 md:gap-12 px-4 md:px-32 py-8 md:py-0 items-center bg-no-repeat bg-cover bg-center">
        <div class="w-full md:w-auto">
          <img src="/images/teco-main-bg2.png" alt="" class="object-contain h-full w-full md:w-[704px]"/>
        </div>
        <div class="flex-1">
          <div class="font-semibold text-4xl md:text-6xl mb-4">技术细节</div>
          <div class="align-multiple-lines-of-text w-full md:w-4/5">核心技术（TOI）（ Transdermal Optical Imaging
            Technology）是TOI技术利用了一些先进的光电容积描记法（PPG）技术，通过分析面部视频中的血流信号来预测健康数据（如血压）。简单来说，TOI的工作原理可以分为以下几个关键步骤：
          </div>
        </div>
      </div>
      <div class="flex flex-col md:flex-row justify-around items-center px-4 md:px-32 gap-6 md:gap-10 py-8 md:py-0"
           style="min-height: 400px;">
        <div class="flex-1/2 w-full md:w-auto">
          <div class="mb-5" style="width: 65px;height: 49px;">
            <img src="/images/teco-1.png" alt="01">
          </div>
          <span class="text-[#323232] text-xl md:text-2xl font-medium p-1"
      ...
      <div
          class="flex flex-col md:flex-row justify-around items-center px-4 md:px-32 gap-6 md:gap-10 py-8 md:py-0 bg-[#F9FAFB]"
          style="min-height: 400px;">
        <div class="flex-1/2 w-full md:w-auto order-2 md:order-1">
          <img src="/images/teco-main-2.png" alt="" class="w-full h-full object-contain">
        </div>
        <div class="flex-1/2 w-full md:w-auto order-1 md:order-2">
      ...
      <div
          class="flex flex-col md:flex-row justify-around items-center px-4 md:px-32 gap-6 md:gap-10 py-8 md:py-0 bg-[#F9FAFB] mb-10"
          style="min-height: 400px;">
        <div class="flex-1/2 w-full md:w-auto order-2 md:order-1">
    ...
    <template v-else-if="selected === 1">
      <div class="flex flex-col lg:flex-row justify-between py-8 md:py-14 gap-8 md:gap-12 px-4 md:px-32">
        <!-- 左侧：健康检测 -->
        <div class="flex-1 space-y-6">
          <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#104A7C]">健康检测</h2>
            <h3 class="text-lg md:text-xl text-gray-400">HEALTH CHECK</h3>
          </div>
          <!-- 左侧卡片：健康检测 -->
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mt-6 md:mt-8 items-stretch">
            <!-- 左列 -->
        ...
        <!-- 右侧：AI风险检测 -->
        <div class="flex-1 space-y-6 md:space-y-8 mt-8 lg:mt-0">
          <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#104A7C]">AI风险检测</h2>
            <h3 class="text-lg md:text-xl text-gray-400">AI RISK ASSESSMENT</h3>
          </div>

          <!-- 上方主图 + 代谢风险 -->
          <div :style="`background-image: url(${bgHealthR1Url})`"
               class="rounded-2xl p-4 shadow flex flex-col md:flex-row gap-4 bg-no-repeat bg-cover bg-center">
            <div class="flex-1">
