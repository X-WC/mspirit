...
function updateSelectedFromQuery(type: unknown) {
  const num = Number(type)
  selected.value = !isNaN(num) ? num : 0
}

watchEffect(() => {
  updateSelectedFromQuery(route.query.type)
})

onBeforeRouteUpdate((to) => {
  updateSelectedFromQuery(to.query.type)
})
</script>
...
<template>
  <div>
    <!-- 顶部横幅 -->
    <div
        class="px-4 xl:px-32 pt-6 xl:pt-10 pb-8 xl:pb-14 bg-no-repeat bg-cover bg-center"
    ...
    <template v-if="selected === 0">
      <div class="flex flex-col xl:flex-row gap-6 xl:gap-12 px-4 xl:px-32 py-7 items-start">
        ...
        <div class="flex flex-col items-center xl:items-start w-full xl:w-[40%]">
          ...
          <img src="/images/market-coverage.png" alt="market" class="mt-6 w-full object-contain" />
        </div>

        <!-- 右侧分类卡片 -->
        <div class="mt-10 xl:mt-20 grid grid-cols-1 xl:grid-cols-2 gap-6 w-full xl:w-[60%]">
          <div
          ...
          >
            ...
            <div class="mt-4 w-full">
              <h3 class="text-xl xl:text-2xl font-bold text-[#323232] border-b border-[#D9D9D9]">
                {{ item.title }}
              </h3>
              <ul class="text-sm text-[#323232] mt-1 leading-relaxed space-y-1 h-24 columns-1 xl:columns-2 overflow-hidden whitespace-nowrap">
                <li v-for="line in item.content" :key="line" class="leading-[32px]">
                  {{ line }}
                </li>
              </ul>
            </div>
      ...
      <!-- 优势轮播 -->
      <section class="pt-8 pb-3 px-4 xl:pl-32 xl:pr-4"
               style="background: linear-gradient(179deg, #9FE7D4 57%, #7BDEFC 100%);">
        <div>
          <h2 class="text-2xl xl:text-4xl font-semibold text-[#104A7C] mb-2 xl:mb-4">优势</h2>
          <p class="text-xl xl:text-2xl text-white mb-6">ADVANTAGE</p>
          <carousel />
        </div>
      </section>
      ...
      <section class="relative py-8 xl:py-12 px-4 xl:px-32">
        <div class="bg-white flex flex-col">
          ...
          <div class="flex flex-wrap xl:flex-row items-end justify-between gap-6 xl:gap-8 relative z-10 mt-6 xl:mt-0">
            <div
                v-for="(text, i) in [
                '社会福利制度对<br>全名健康的巨大投入',
                '不可逆的AI趋势',
                '个体对健康<br>关注的提高',
                '老龄化社会的<br>健康需求',
                '个性化健康<br>管理的需求',
                '企业对员工福利<br>和健康关注度的增加'
              ]"
                :key="i"
                class="flex flex-col items-center w-[50%] xl:w-[13%]"
            >
              <div class="rounded-full overflow-hidden flex items-center justify-center">
                <img :src="`/images/market-opportunity-${i + 1}.png`" class="object-cover w-full h-full" />
              </div>
              <div class="text-center text-[#104A7C] font-bold text-sm xl:text-base mt-2" v-html="text" />
            </div>
          </div>
        </div>

        <img src="/images/market-opportunity-bg.png" class="absolute right-0 top-[20%] z-5 w-40 xl:w-80 hidden xl:block" />
        <div class="w-full h-[90px] bg-[#F9F9F9]" style="margin-top: -70px;" />
      </section>
    </template>
